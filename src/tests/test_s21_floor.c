...
START_TEST(test_floor_positive_with_fraction) {
  const char *num_str = "79228162514264337.23423423";
  ...
  int got_status = s21_floor(num, &got_result);
...
START_TEST(test_floor_negative_with_fraction) {
  ...
  const char *expected_result_str = "-79228162514264338";
  int expected_status = 0;
...
START_TEST(test_floor_small_negative) {
  const char *num_str = "-0.000000001";
  NEW_DECIMAL(num, num_str);
...
Suite *suite_s21_floor(void) {
  ...
  tcase_add_test(tcase_core, test_floor_positive_with_fraction);
  tcase_add_test(tcase_core, test_floor_negative_with_fraction);
  tcase_add_test(tcase_core, test_floor_positive_integer);
  tcase_add_test(tcase_core, test_floor_small_positive);
  tcase_add_test(tcase_core, test_floor_small_negative);
  tcase_add_test(tcase_core, test_floor_zero);
